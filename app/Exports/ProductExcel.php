...
class ProductExcel implements ShouldAutoSize, FromView, WithEvents
{
  use Exportable;
  ...
  {
    $this->rows = SaleList::reportLists($this->filter);;
  ...
  public function registerEvents(): array
  {
    return [AfterSheet::class => function (AfterSheet $event) {
      // header
      $event->sheet->getDelegate()->getStyle('A1:F1')->getAlignment()->setHorizontal('center');
      $event->sheet->getDelegate()->getStyle('A1:F1')->getFont()->setName('Moul');
  ...
  public function view(): View
  {
    $data = $this->rows;
    $filter = $this->filter;
    return view('prints.excel.product_excel', [
      'filter' => $filter,
      'data' => $data,
    ]);
  }
