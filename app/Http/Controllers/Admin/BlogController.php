...
class BlogController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($blog->save()) {
            ...
            if ($image) {
                $image = StringHelper::uploadImage($image, 'blog', IsHasThumbnail::YES['id'], IsCropImage::NO['id']);
                $blog->image = "$image";
                $blog->save();
            }
        }
    ...
    public function delete(Request $request)
    {
        ...
        if ($blog->delete()) {
            //Delete Logo
            StringHelper::deleteImage($blog->logo, Blog::logoPath, Blog::thumbnailPath);
        }
        DB::commit();
        return response()->json([
            'data' => $blog,
