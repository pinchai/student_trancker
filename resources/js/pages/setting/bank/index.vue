              ...
              <template v-slot:cell(image)="row">
                ...
                <div v-else>
                  <img
                    ...
                    class="img-thumbnail rounded"
                    :id="row.item.image"
                    :src="'/images/bank/' + row.item.image"
                  >
                  <b-tooltip
                    ...
                    variant="primary"
                  >
                    <img class="img-thumbnail" :src="'/images/bank/' + row.item.image">
                  </b-tooltip>
                </div>
              </template>
              <template v-slot:cell(is_show)="row">
                <b-badge
                  v-if="row.item.is_show == 1"
                  variant="primary"
                >
                  Show
                </b-badge>
    ...
    deleteItem() {
      this.$fire({
        ...
        if (result.value) {
          let vm = this;
          axios
            .post("/bank/delete", {id: this.selectedItem.id})
            .then(function (response) {
              vm.fetchRecord();
              vm.$notify({
                group: "message",
