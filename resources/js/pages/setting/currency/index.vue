          ...
          <b-col cols="12">
            ...
            >
              <template v-slot:cell(is_default)="row">
                <div v-if="row.item.is_default === 1">
                  <span v-if="$i18n.locale == 'en'">Yes</span>
                  <span v-if="$i18n.locale == 'kh'"
                  >ប្រើជាលំនាំដើម</span
                  >
                </div>
                <div v-if="row.item.is_default === 0">
  ...
  computed: {
    ...
    header() {
      return [
        ...
        {
          key: "buy_in_price",
          label: this.$t("buy_in_price"),
          sortable: true
        },
        {
          key: "sell_out_price",
          label: this.$t("sell_out_price"),
          sortable: true
        },
        {
          key: "is_default",
    ...
    deleteItem() {
      this.$fire({
        ...
        if (result.value) {
          let vm = this;
          axios
            .post("/currency/delete", {id: this.selectedItem.id})
            .then(function (response) {
              ...
              if (vm.$helpers.nullToVoid(itemData.data) != "") {
                if (itemData.data.is_default == 1) {
                  vm.$store.dispatch(
                    "setDefaultCurrency",
                    itemData.data
                  );
                ...
                vm.$store.dispatch("setDefaultCurrency", []);
              }
