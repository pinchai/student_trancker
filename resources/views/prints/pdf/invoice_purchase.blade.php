  ...
  <div class="wrapper">
    ...
    <div style="width: 59%;float: left; text-align: right; font-weight: bolder">
      <div style="width: 200px ;float: right; text-align: center; color: black;">
        <div>
          <span style="font-size: 25px; font-family: muol;">វិក័យប័ត្រទិញ</span>
        </div>
      ...
      <div>ឈ្មោះអ្នកផ្គត់ផ្គង់: {{ $data['supplier_name'] }}</div>
      <div>លេខទូរស័ព្ទ: {{ $data['supplier_phone'] }}</div>
      <div>អាសយដ្ឋាន: {{ $data['supplier_address'] }}</div>
    </div>
          ...
          <tr>
            ...
            <td align="right" style="font-size: 15px">
              @if($item->total_cost != 0)
                {{ $item->symbol }}
                {{ $item->currency_symbol }} {{ App\Helpers\StringHelper::currency($item->total_cost) }}
              @else
                {{ '--' }}
              @endif
            </td>
          ...
          <td style="font-size: 15px; text-align: right">
            <label>
              @if($data['delivery_cost'] > 0)
                {{ $data['symbol'] }}
                {{ App\Helpers\StringHelper::currency($data['delivery_cost']) }}
              @else
                {{ 'Free' }}
              @endif
            </label>
